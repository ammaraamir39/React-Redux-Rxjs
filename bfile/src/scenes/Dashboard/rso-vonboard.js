...
class RSOVonboard extends Component {
    state = {
        loading: false,
        period: 'mtd',
        ...
        buckets: [
            {index: 0, title: "My Claimed Calls", calls: [], loading: false, page: 1, total_pages: 0, filters: {
                search: ("rso_vo_box1_search" in window.localStorage) ? window.localStorage.rso_vo_box1_search : '',
                agency: ("rso_vo_box1_agency" in window.localStorage) ? window.localStorage.rso_vo_box1_agency : ''
            }},
            {index: 1, title: "Due Calls", calls: [], loading: false, page: 1, total_pages: 0, filters: {
                search: ("rso_vo_box2_search" in window.localStorage) ? window.localStorage.rso_vo_box2_search : '',
                agency: ("rso_vo_box2_agency" in window.localStorage) ? window.localStorage.rso_vo_box2_agency : ''
            }},
            {index: 2, title: "Attempted Calls", calls: [], loading: false, page: 1, total_pages: 0, filters: {
                search: ("rso_vo_box3_search" in window.localStorage) ? window.localStorage.rso_vo_box3_search : '',
                agency: ("rso_vo_box3_agency" in window.localStorage) ? window.localStorage.rso_vo_box3_agency : ''
            }},
            {index: 3, title: "Calendar Invites", calls: [], loading: false, page: 1, total_pages: 0, filters: {
                search: ("rso_vo_box4_search" in window.localStorage) ? window.localStorage.rso_vo_box4_search : '',
                agency: ("rso_vo_box4_agency" in window.localStorage) ? window.localStorage.rso_vo_box4_agency : ''
            }},
            {index: 4, title: "Upcoming Calls", calls: [], loading: false, page: 1, total_pages: 0, filters: {
                search: ("rso_vo_box5_search" in window.localStorage) ? window.localStorage.rso_vo_box5_search : '',
                agency: ("rso_vo_box5_agency" in window.localStorage) ? window.localStorage.rso_vo_box5_agency : ''
            }},
            {index: 5, title: "Completed Calls", calls: [], loading: false, page: 1, total_pages: 0, filters: {
                search: ("rso_vo_box6_search" in window.localStorage) ? window.localStorage.rso_vo_box6_search : '',
                agency: ("rso_vo_box6_agency" in window.localStorage) ? window.localStorage.rso_vo_box6_agency : ''
            }},
            {index: 6, title: "Not Reached Archived", calls: [], loading: false, page: 1, total_pages: 0, filters: {
                search: ("rso_vo_box7_search" in window.localStorage) ? window.localStorage.rso_vo_box7_search : '',
                agency: ("rso_vo_box7_agency" in window.localStorage) ? window.localStorage.rso_vo_box7_agency : ''
            }},
            {index: 7, title: "Sent For Financial", calls: [], loading: false, page: 1, total_pages: 0, filters: {
                search: ("rso_vo_box8_search" in window.localStorage) ? window.localStorage.rso_vo_box8_search : '',
                agency: ("rso_vo_box8_agency" in window.localStorage) ? window.localStorage.rso_vo_box8_agency : ''
            }},
            {index: 8, title: "Not Interested", calls: [], loading: false, page: 1, total_pages: 0, filters: {
                search: ("rso_vo_box9_search" in window.localStorage) ? window.localStorage.rso_vo_box9_search : '',
                agency: ("rso_vo_box9_agency" in window.localStorage) ? window.localStorage.rso_vo_box9_agency : ''
            }}
        ],
        columns: [{
            title: 'Name',
    ...
    componentDidMount = () => {
        ...
        if ("rso_vo_period" in window.localStorage) {
            if (window.localStorage.rso_vo_period === 'from_to') {
                this.setState({
                    start_date: window.localStorage.rso_vo_start_date,
                    end_date: window.localStorage.rso_vo_end_date,
                    period: window.localStorage.rso_vo_period
                })
            } else if (window.localStorage.rso_vo_period === 'all_bfiles') {
                this.setState({
                    period: window.localStorage.rso_vo_period
                })
            }
    ...
    changePeriod = (e) => {
        ...
        if (value === "mtd") {
            window.localStorage.rso_vo_start_date = null;
            window.localStorage.rso_vo_end_date = null;
            window.localStorage.rso_vo_period = value;
            ...
            window.localStorage.rso_vo_start_date = new Date('2010-01-01');
            window.localStorage.rso_vo_end_date = new Date(today.getFullYear(), today.getMonth()+1, 1);
            window.localStorage.rso_vo_period = value;
    ...
    agencyChange = (ind, agency_id) => {
        ...
        for (let i = 0; i < buckets.length; i++) {
            if (buckets[i].index === ind) {
                ...
                window.localStorage["rso_vo_box" + (i+1) + "_agency"] = agency_id;
                break;
            }
        }
        this.setState({ buckets }, () => {
    ...
    onChangeDateRange = (date, dateString) => {
        window.localStorage.rso_vo_start_date = date[0];
        window.localStorage.rso_vo_end_date = date[1];
        window.localStorage.rso_vo_period = "from_to";
    ...
    loadBucket = (ind, api, filters, page) => {
        ...
        if (more_filters.agency && more_filters.agency !== "") {
