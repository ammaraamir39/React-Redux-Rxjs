...
class Step extends Component {
    render() {
        ...
        home_list.push({ title: 'Home', icon: Home, value: recommended_limit_2 })
        let name = null;
        let icon = null;
        let value = null;
        if (wizard.secondary_home_type) {
            ...
            if (secondary_home_type) {
                for(let i=0, len = secondary_home_type.length; i < len; i++) {
                    ...
                    home_list.push({ title: name, icon: icon, value: recommended_limit_2 })
                }
            }
        }

        auto_list.push({ title: 'Auto', icon: Auto, value: recommended_limit })
        ...
        if (wizard.secondary_vehicles) {
            ...
            if (secondary_vehicles) {
                for(let i = 0, len = secondary_vehicles.length; i < len; i++) {
                    ...
                    auto_list.push({ title: name, icon: icon, value: value })
                }
            }
        }
