...
class AgencyManagerNew extends Component {
    state = {
        loading: true,
        users: [],
        page: 1,
    ...
    loadUsers = (page) => {
        ...
        filters.push({"name":"user_type","op":"==","val":"AGENCY_MANAGER"});
        ...
        axios.get(api).then((res) => {
            console.log("Res object - > ",res.data)
            this.setState({
                loading: false,
    ...
    updateAgencyAssoc(agency_id, checked) {
        ...
        var val = 0;
        if (checked) val = 1;
        axios.post("/api/update_am_agency_assoc/" + this.state.user.id + "/" + agency_id + "/" + val).then(() => {
...
export default AgencyManagerNew;
