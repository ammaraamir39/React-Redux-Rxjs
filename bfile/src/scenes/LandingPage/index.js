...
import SetupAppointmentModal from './parts/setup-appointment-modal';
...
import AddNoteModal from './parts/add-note-modal';
...
class LandingPage extends Component {
    ...
    edit = () => {
        this.props.history.push('/wizard/step/1/' + this.state.bfile_id);
    }
    goTo = (path) => {
        ...
        return (
            <div id="landingPage">
                    ...
                    <Row gutter={16}>
                        <Col md={12} span={24}>
                            <div className="bfile_title_contact_info">
                                ...
                                {bfile.phone2 !== null && bfile.phone2 !== '' ? (
                                    <div className="phone">
                                        <Icon type="phone" style={{marginRight:10}} />
                            ...
                            <div className="profileButtons">
                                {user.user_type === 'EA' || user.user_type === 'VONBOARDER' || user.user_type === 'LSP' ? (
                                    <Button onClick={this.edit.bind(this)}><Translate text={`Edit / Update`} /></Button>
                                ) : null}
                                <ActionMenu bfile={bfile} user={user} history={this.props.history} update={(bfile) => this.setState({ bfile })} refresh={this.init.bind(this)} />
                            </div>
                        </Col>
                        <Col md={12} span={24}>
                            <div id="notes">
                                <h3><Icon type="notification" style={{marginRight:10}} /> <Translate text={`Notes`} />:</h3>
                                <List
                                    loading={notes_loading}
                                    itemLayout="horizontal"
                                    dataSource={few_notes}
                                    renderItem={item => (
                                        <List.Item>
                                            <List.Item.Meta
                                ...
                                />
                                {notes.length > 2 ? (
                                    <Button onClick={() => this.setState({ show_notes_modal: true })}><Translate text={`Show More Notes`} /> <Icon type="down" style={{marginRight: 10}} /> </Button>
                                ) : null}
                                <Button onClick={() => this.setState({ add_note_modal: true })}><Translate text={`Add a Note`} /> </Button>
                            </div>
                        </Col>
                    </Row>
                ...
                {this.state.history.length > 1 ? (
                    <Card title={<Translate text={`History`} />} className="bfileHistory" style={{marginBottom:20}} type="inner" bordered={false}>
                        <List
                            loading={loading}
                            itemLayout="horizontal"
                                ...
                                <List.Item>
                                    <Link to={"/customer/" + item.id}>
                                        <List.Item.Meta
                                            ...
                                            description={moment(item.created_on).format('MM/DD/YYYY')}
                                            className={(item.id === bfile.id) ? 'current' : ''}
                                        />
                                    </Link>
                                </List.Item>
                            )}
                ...
                <NotesModal
                    showModal={this.state.show_notes_modal}
                    hideModal={() => this.setState({ show_notes_modal: false })}
                    notes={notes}
                ...
                {!loading ? (
                    <AddNoteModal
                        showModal={this.state.add_note_modal}
                        hideModal={() => this.setState({ add_note_modal: false })}
                        bfile={bfile}
                        user={user}
                    />
                ) : null}
